"""
Generate a single-file HTML review dashboard for the 60-AOI human review.
Opens in any browser — no server needed.
User marks PASS/FAIL per route, then exports the filled CSV.
"""
import json, csv, math
from pathlib import Path

BASE      = Path('.')
CSV_IN    = BASE / 'data' / 'human_review' / 'route_review.csv'
OUT_HTML  = BASE / 'data' / 'human_review' / 'review_dashboard.html'

# Load sampled AOIs from CSV
with open(CSV_IN) as f:
    reader = csv.DictReader(f)
    rows = list(reader)

# Group rows by aoi_id
from collections import defaultdict, OrderedDict
aoi_rows = OrderedDict()
for r in rows:
    aoi_rows.setdefault(r['aoi_id'], []).append(r)

sampled_aois = list(aoi_rows.keys())
print(f'AOIs to review: {len(sampled_aois)}')

# Load evacuation routes for each sampled AOI
def load_routes(aoi_id):
    p = BASE / 'data' / 'annotations' / aoi_id / 'evacuation_routes.json'
    return json.load(open(p)) if p.exists() else {}

def load_bbox(aoi_id):
    aois = json.load(open(BASE / 'data' / 'aoi_list.json'))
    a = next((x for x in aois if x['aoi_id'] == aoi_id), None)
    return a['bbox'] if a else None  # [minx, miny, maxx, maxy]

# Build per-AOI data for JS
aoi_data = []
for aoi_id in sampled_aois:
    routes_json = load_routes(aoi_id)
    bbox = load_bbox(aoi_id)
    center_lat = (bbox[1] + bbox[3]) / 2 if bbox else 0
    center_lon = (bbox[0] + bbox[2]) / 2 if bbox else 0

    epochs_data = []
    csv_rows = aoi_rows[aoi_id]
    # group csv rows by epoch
    epoch_csv = defaultdict(list)
    for r in csv_rows:
        epoch_csv[r['epoch']].append(r)

    for epoch_label, rlist in routes_json.items():
        route_items = []
        for i, r in enumerate(rlist, 1):
            coords = r.get('geometry', {}).get('coordinates', [])
            csv_r = epoch_csv.get(epoch_label, [{}] * i)
            route_items.append({
                'route_id': i,
...
#progress-fill {{ height: 4px; background: #4caf50; border-radius: 2px; width: 0%; transition: width 0.3s; }}
...
.status-todo {{ background: #555; }}
.aoi-label {{ flex: 1; font-size: 12px; }}
.aoi-count {{ font-size: 10px; color: #888; }}
...
.route-card h4 {{ font-size: 12px; margin-bottom: 6px; color: #aaa; }}
...
.btn-pass.selected {{ background: #4caf50; color: #fff; }}
.btn-fail.selected {{ background: #ef5350; color: #fff; }}
...
<div id="sidebar">
  <div id="sidebar-header">TDRD Route Review</div>
...
<div id="main">
  ...
  <div id="content">
    <div id="map"></div>
    <div id="review-panel"><p style="color:#888;font-size:12px">Select an AOI from the list to begin review.</p></div>
  </div>
  <div id="nav-btns">
    <button class="nav-btn" onclick="navAOI(-1)">&#8592; Previous AOI</button>
...
// State: verdicts[aoi_id][epoch][route_id] = {{verdict, comment}}
const verdicts = {{}};

// Init from original CSV (in case re-opened with some filled)
ORIGINAL_ROWS.forEach(r => {{
  if (!verdicts[r.aoi_id]) verdicts[r.aoi_id] = {{}};
  if (!verdicts[r.aoi_id][r.epoch]) verdicts[r.aoi_id][r.epoch] = {{}};
  verdicts[r.aoi_id][r.epoch][r.route_id] = {{
    verdict: r.verdict || '',
    comment: r.comment || ''
  }};
}});
...
function buildSidebar() {{
  const el = document.getElementById('aoi-list');
  el.innerHTML = '';
  AOI_DATA.forEach((aoi, idx) => {{
    const done = countDone(aoi.aoi_id);
    const total = countTotal(aoi.aoi_id);
    const statusClass = done === 0 ? 'status-todo' : (done === total ? 'status-done' : 'status-partial');
    const div = document.createElement('div');
    div.className = 'aoi-item' + (idx === currentAOI ? ' active' : '');
    div.innerHTML = `<div class="aoi-status ${{statusClass}}"></div>
      <span class="aoi-label">${{aoi.aoi_id}}</span>
      <span class="aoi-count">${{done}}/${{total}}</span>`;
    div.onclick = () => {{ currentAOI = idx; currentEpoch = 0; render(); }};
    el.appendChild(div);
  }});
}}

function countTotal(aoi_id) {{
  return ORIGINAL_ROWS.filter(r => r.aoi_id === aoi_id).length;
}}

function countDone(aoi_id) {{
  const v = verdicts[aoi_id] || {{}};
  let n = 0;
...
function render() {{
  ...
  // Epoch tabs
  const tabsEl = document.getElementById('epoch-tabs');
  tabsEl.innerHTML = '';
  ...
  // Sidebar highlight
  document.querySelectorAll('.aoi-item').forEach((el, i) => {{
    el.classList.toggle('active', i === currentAOI);
  }});
...
function renderEpoch() {{
  const aoi = AOI_DATA[currentAOI];
  if (aoi.epochs.length === 0) {{ renderPanel([]); return; }}
  ...
  if (aoi.bbox) {{
    ...
    const rect = L.rectangle([[miny, minx], [maxy, maxx]], {{color:'#fff', weight:1, fill:false, dashArray:'4'}}).addTo(map);
  ...
  ep.routes.forEach((r, i) => {{
    ...
    // Start marker
    const start = L.circleMarker(latlngs[0], {{radius:6, color:'#fff', fillColor:'#333', fillOpacity:1, weight:2}}).addTo(map);
    // End marker
    const end = L.circleMarker(latlngs[latlngs.length-1], {{radius:7, color:'#fff', fillColor:color, fillOpacity:1, weight:2}}).addTo(map);
    routeLayers.push(start, end);
  }});

  // Fit map
  if (ep.routes.length > 0 && ep.routes[0].coords && ep.routes[0].coords.length > 0) {{
    const allCoords = ep.routes.flatMap(r => (r.coords||[]).map(c => [c[1], c[0]]));
...
function renderPanel(routes, aoi_id, epoch) {{
  ...
  if (routes.length === 0) {{
    panel.innerHTML = '<p style="color:#888;font-size:12px">No routes for this epoch.</p>';
    return;
  }}

  const colors = ['#e94560', '#4fc3f7', '#81c784', '#ffb74d', '#ce93d8'];
  routes.forEach((r, i) => {{
    const v = (verdicts[aoi_id]?.[epoch]?.[String(r.route_id)]) || {{verdict:'', comment:''}};
...
function updateProgress() {{
  let done = 0;
  const total = ORIGINAL_ROWS.length;
  ORIGINAL_ROWS.forEach(r => {{
    const v = verdicts[r.aoi_id]?.[r.epoch]?.[String(r.route_id)];
    if (v && v.verdict) done++;
  }});
...
function navAOI(dir) {{
  ...
  render();
  // Scroll sidebar
  const items = document.querySelectorAll('.aoi-item');
  items[currentAOI]?.scrollIntoView({{block:'nearest'}});
}}
...
function exportCSV() {{
  ...
  const lines = ORIGINAL_ROWS.map(r => {{
    ...
    const comment  = (v?.comment || '').replace(/"/g, '""');
...
print('Open it in Chrome/Firefox — no server needed.')
